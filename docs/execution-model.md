...
defineSpecs(dir)
  └─ for each .md file: parse into a Spec (title, background, scenarios, line numbers)
      └─ for each scenario:
          ├─ resolve every step against the registry
          ├─ union the fixtures those steps destructure
...
| `generate` | Spec × registry → Playwright tests, including the per-scenario signature. |
| `reporter` | Spec-grouped output, failing step, `.md` location. |
| `create` | Binds the above to a caller's `test` and returns `{ step, defineSpecs }`. |
